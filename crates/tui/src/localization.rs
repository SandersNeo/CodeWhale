...
fn chinese_simplified(id: MessageId) -> Option<&'static str> {
    ...
    static MAP: LazyLock<HashMap<String, String>> = LazyLock::new(|| {
        serde_json::from_str(include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/locales/zh-Hans.json"))).expect("valid zh-Hans.json translations")
    });
    MAP.get(&format!("{id:?}")).map(|s| s.as_str())
}
